...
import { getRequestStatusLabel, requestStatusLabels } from '../../utils/requestStatus'
      ...
      <div className={styles.workItemGrid}>
        <div className={styles.workItemMain}>
          ...
          <article className={styles.panel}>
            ...
            {activeTab === 'activity' && <div>{request.actividad.map((item, index) => <div className={styles.activityItem} key={`${item.fecha}-${index}`}><span className={styles.activityDot} /><div><strong>{item.usuario}</strong> {item.descripcion.toLowerCase()}<small>{formatDate(item.fecha)}</small>{(item.valorAnterior || item.valorNuevo) && <p><del>{getRequestStatusLabel(item.valorAnterior)}</del> → <strong>{getRequestStatusLabel(item.valorNuevo)}</strong></p>}</div></div>)}</div>}
            ...
            {activeTab === 'history' && <div>{request.historial.map((item, index) => <div className={styles.timeline} key={`${item.fecha}-${index}`}><strong>{item.estadoAnterior ? requestStatusLabels[item.estadoAnterior] : 'Inicio'} → {requestStatusLabels[item.estadoNuevo]}</strong><small>{item.usuario} · {formatDate(item.fecha)}</small>{item.comentario && <p>{item.comentario}</p>}</div>)}</div>}
          </article>
        </div>
        ...
        <aside className={`${styles.panel} ${styles.workItemFields}`}>
          <h3>Información</h3>
          <label>Estado
            {canManage && request.estado !== 'Cerrada' ? <select value={pendingStatus ?? request.estado} onChange={(event) => { const value = event.target.value as RequestStatus; setPendingStatus(value === request.estado ? null : value) }}><option value={request.estado}>{requestStatusLabels[request.estado]}</option>{allowedTransitions[request.estado].map((status) => <option value={status} key={status}>{requestStatusLabels[status]}</option>)}</select> : <span className={styles.readonlyField}>{requestStatusLabels[request.estado]}</span>}
          </label>
