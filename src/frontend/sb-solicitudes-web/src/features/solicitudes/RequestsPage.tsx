...
import { requestStatusOptions } from '../../utils/requestStatus'
...
export function RequestsPage() {
  ...
  const view = location.pathname.endsWith('/asignadas')
    ? 'assigned'
    : location.pathname.endsWith('/disponibles') ? 'available' : 'all'
  ...
  const load = useCallback(async () => {
    ...
    if (view === 'assigned' && session) query.set('responsableId', session.usuarioId)
    else if (view === 'available') query.set('sinAsignar', 'true')
    else if (debouncedFilters.assigneeId) query.set('responsableId', debouncedFilters.assigneeId)
  ...
  }, [debouncedFilters, pageNumber, session, view])
  ...
  return (
    <section>
      <div className={styles.pageHeader}><p>{view === 'assigned' ? 'Solicitudes bajo su responsabilidad.' : view === 'available' ? 'Solicitudes sin responsable disponibles para gestión.' : session?.rol === 'Solicitante' ? 'Creación y seguimiento de sus solicitudes.' : 'Consulta y seguimiento de todas las solicitudes.'}</p>{!isAnalyst && <Link className={styles.primaryButton} to="/solicitudes/nueva">Nueva solicitud</Link>}</div>
      <div className={styles.filters}>
        ...
        {isAdministrator && <select value={assigneeId} onChange={(event) => setAssigneeId(event.target.value)}><option value="">Todos los responsables</option>{users.map((user) => <option value={user.id} key={user.id}>{user.nombre}</option>)}</select>}
